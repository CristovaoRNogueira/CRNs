...
// CORREÇÃO DO ERRO FATAL: Garante que classificacao seja sempre array
        ...
        <aside class="offers-sidebar">
            <div class="filter-box">
                ...
                <form action="<?php echo esc_url( get_permalink() ); ?>" method="GET">
                    ...
                    <?php 
                    // Usa a nova função para buscar apenas classificações válidas
                    $perfis = function_exists('crns_get_valid_classifications') ? crns_get_valid_classifications($filter_cat) : [];
                    
                    // Só mostra o grupo se houver perfis disponíveis
                    if( !empty($perfis) ):
                    ...
                    <div class="filter-group <?php echo $class_active; ?>">
                        ...
                        <div class="filter-options">
                            <?php foreach($perfis as $p): ?>
                                <label>
                                    <input type="checkbox" name="classificacao[]" value="<?php echo $p->slug; ?>" <?php if(in_array($p->slug, $filter_class)) echo 'checked'; ?>>
                                    <?php echo $p->name; ?>
                                </label>
                            <?php endforeach; ?>
                        </div>
            ...
            <div class="offers-grid">
                    ...
                    <?php while( $offers->have_posts() ) : $offers->the_post();
                        ...
                        foreach($priority as $key) {
                            ...
                            if(isset($all_meta[$key][0]) && $all_meta[$key][0]) {
                                $specs_html .= '<span>' . esc_html($all_meta[$key][0]) . '</span> • '; $c++;
                            }
                        }
                ...
                <?php else: ?>
                    <div class="no-results" style="grid-column: 1 / -1; text-align:center; padding: 40px;">
                        <p style="font-size:1.2rem">🚫 Nenhum produto com esses filtros.</p>
